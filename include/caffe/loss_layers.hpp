...
/**
 * @brief Computes the classification accuracy for a multilabel one-of-many
 *        classification task, and output the accuracy of each label
 */
template <typename Dtype>
...
/**
 * @brief compute the prediction accuracy of a net with a cross entropy
 *        loss layer.
 */
template <typename Dtype>
class CrossEntropyAccuracyLayer : public Layer<Dtype> {
 public:
  /**
   * @param param provides AccuracyParameter accuracy_param,
   *     with AccuracyLayer options:
   ...
   *     correct if the correct label is among the top 5 predicted labels.
   */
  explicit CrossEntropyAccuracyLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "CrossEntropyAccuracy"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline int ExactNumTopBlobs() const { return -1; }
  virtual inline int MinTopBlobs() const {return 1;}
  virtual inline int MaxTopBlobs() const {return 3;}
